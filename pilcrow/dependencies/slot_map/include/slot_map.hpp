...
public:
  using key_size_type
    = std::remove_reference_t<decltype(std::get<0>(std::declval<Token>()))>;
  using key_generation_type
    = std::remove_reference_t<decltype(std::get<1>(std::declval<Token>()))>;
  ...
  // Note:
  // slot_map requires a container providing an iterator satisfying the
  // constraints of RandomAccessIterator
  ...
  static_assert(sizeof(key_size_type)
                  <= sizeof(typename Container<T>::size_type),
                "The size of the type of get<0>() for the Token type must be "
                "at most Container::size_type.");
  static_assert(std::tuple_size<Token>::value == 2,
                "The token type for a slot_map must be decomposable into 2 "
                "integer-like types.");
  static_assert(
    std::is_same<typename std::iterator_traits<iterator>::iterator_category,
                 std::random_access_iterator_tag>::value,
    "slot_map requires the adapted container to provide random access "
    "iterators.");
  ...
  constexpr reference at(const key_type &key);  // bounds+generation checked,
                                                // O(1) in all cases
  constexpr const_reference at(const key_type &key) const;  // bounds+generation
                                                            // checked, O(1) in
                                                            // all cases
  // operator[] checks generation counter and has undefined behavior for a key
  // which fails the check.
  constexpr reference operator[](const key_type &key);  // generation checked,
                                                        // O(1) in all cases
  constexpr const_reference
  operator[](const key_type &key) const;  // generation checked, O(1) in all
                                          // cases
  // returns an iterator to the element if it is found, returns an iterator to 1
  // past the last element otherwise.
  constexpr iterator find(const key_type &key);  // generation checked, O(1) in
                                                 // all cases
  constexpr const_iterator
  find(const key_type &key) const;  // generation checked, O(1) in all cases
  ...
  constexpr reference
  find_unchecked(const key_type &key);  // unsafe, no checks, O(1) in all cases
  constexpr const_reference
  find_unchecked(const key_type &key) const;  // unsafe, no checks, O(1) in all
                                              // cases
  ...
  constexpr void reserve(size_type n);   // O(N), O(1) if(empty() &&
                                         // n<slot_capacity() ), or if(n <
                                         // capacity())

  // Capacity unique to slot_map
  constexpr void reserve_slots(size_type n);   // O(N), O(1) if(capacity_slots()
                                               // > n)
  constexpr size_type capacity_slots() const;  // O(1) in all cases

  // Modifiers equivalent to map
  constexpr void     clear();                        // O(N)
  constexpr key_type insert(const_reference value);  // O(1), O(N) when
                                                     // allocation is required
  constexpr key_type insert(value_type &&value);  // O(1), O(N) when allocation
                                                  // is required
  template <typename... Args>
  constexpr key_type emplace(Args &&... args);  // O(1), O(N) when allocation is
                                                // required
  constexpr iterator erase(iterator pos);       // O(1) in all cases
  constexpr iterator erase(iterator first, iterator last);  // O(1) per element
                                                            // being erased,
                                                            // worst case O(N)
  constexpr iterator erase(const_iterator pos);             // O(1) in all cases
  constexpr iterator erase(const_iterator first,
                           const_iterator last);   // O(1) per element being
                                                   // erased, worst case O(N)
  constexpr size_type erase(const key_type &key);  // O(1) in all cases. Will
                                                   // contain an iterator if an
                                                   // element was erased
...
private:
  ...
  constexpr const key_generation_type &
  key_generation(const key_type &key) const;
...
slot_map<T, Token, Container>::begin() {
  return std::begin(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::iterator
slot_map<T, Token, Container>::end() {
  return std::end(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_iterator
slot_map<T, Token, Container>::begin() const {
  return std::begin(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_iterator
slot_map<T, Token, Container>::end() const {
  return std::end(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_iterator
slot_map<T, Token, Container>::cbegin() const {
  return std::cbegin(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_iterator
slot_map<T, Token, Container>::cend() const {
  return std::cend(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::reverse_iterator
slot_map<T, Token, Container>::rbegin() {
  return std::rbegin(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::reverse_iterator
slot_map<T, Token, Container>::rend() {
  return std::rend(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reverse_iterator
slot_map<T, Token, Container>::rbegin() const {
  return std::rbegin(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reverse_iterator
slot_map<T, Token, Container>::rend() const {
  return std::rend(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reverse_iterator
slot_map<T, Token, Container>::crbegin() const {
  return std::rbegin(m_data);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reverse_iterator
slot_map<T, Token, Container>::crend() const {
  return std::rend(m_data);
}
#pragma endregion
...
template <typename T, typename Token, template <typename...> class Container>
constexpr const typename slot_map<T, Token, Container>::container_type &
slot_map<T, Token, Container>::data() const {
  return m_data;
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::reference
slot_map<T, Token, Container>::at(const key_type &key) {
  const auto &index{this->key_index(key)};
  const auto &generation{this->key_generation(key)};
  if(index >= m_slots.size()
     || generation != key_generation(*(m_slots.begin() + index)))
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reference
slot_map<T, Token, Container>::at(const key_type &key) const {
  const auto &index{key_index(key)};
  const auto &generation{key_generation(key)};
  if(index >= m_slots.size()
     || generation != this->key_generation(*(std::begin(m_slots) + index)))
    throw std::range_error("Invalid key used with slot_map::at.");
  else
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reference
  slot_map<T, Token, Container>::operator[](const key_type &key) const {
  const auto &index{this->key_index(key)};
  const auto &generation{this->key_generation(key)};
  if(index >= m_slots.size()
     || generation != this->key_generation(*(std::begin(m_slots) + index)))
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::reference
  slot_map<T, Token, Container>::operator[](const key_type &key) {
  const auto &index{this->key_index(key)};
  const auto &generation{this->key_generation(key)};
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::iterator
slot_map<T, Token, Container>::find(const key_type &key) {
  const auto &index{this->key_index(key)};
  const auto &generation{this->key_generation(key)};
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_iterator
slot_map<T, Token, Container>::find(const key_type &key) const {
  const auto &index{this->key_index(key)};
  const auto &generation{this->key_generation(key)};
  if(generation != this->key_generation(*(m_slots.begin() + index)))
    return this->end();
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::reference
slot_map<T, Token, Container>::find_unchecked(const key_type &key) {
  return *(std::begin(m_data) + this->element_index(key));
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::const_reference
slot_map<T, Token, Container>::find_unchecked(const key_type &key) const {
  return *(std::cbegin(m_data) + this->element_index(key));
}
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::size_type
slot_map<T, Token, Container>::size() const {
  return static_cast<size_type>(m_data.size());
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::size_type
slot_map<T, Token, Container>::max_size() const {
  return static_cast<size_type>(m_data.max_size());
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::size_type
slot_map<T, Token, Container>::capacity() const {
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::size_type
slot_map<T, Token, Container>::capacity_slots() const {
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::key_type
slot_map<T, Token, Container>::finish_inserting_last_element() {
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::key_type
slot_map<T, Token, Container>::insert(const_reference value) {
  this->pre_insert();
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::key_type
slot_map<T, Token, Container>::insert(value_type &&value) {
  this->pre_insert();
...
template <typename... Args>
constexpr typename slot_map<T, Token, Container>::key_type
slot_map<T, Token, Container>::emplace(Args &&... args) {
  this->pre_insert();
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::iterator
slot_map<T, Token, Container>::erase(iterator pos) {
  if(pos == this->end())
    return pos;
    ...
    *(m_erase_helper.begin() + dist) = m_erase_helper.back();
    m_erase_helper.pop_back();
    this->key_index(*(std::begin(m_slots) + update_slot))
      = static_cast<key_size_type>(dist);
    return pos;
  }
}
...
//  static_assert(false, "erase() needs to be implemented.");
  ...
  if(last == this->end()) {
    ...
    // then everything remaining to be erased reaches the end and they can be
    // erased in reverse order
    return first;
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::iterator
slot_map<T, Token, Container>::erase(const_iterator pos) {
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::iterator
slot_map<T, Token, Container>::erase(const_iterator first,
                                     const_iterator last) {
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::size_type
slot_map<T, Token, Container>::erase(const key_type &key) {
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::size_type
slot_map<T, Token, Container>::pop_head() {
  if(this->size() == this->capacity()) this->grow();
  key_size_type next_free_head{
    this->key_index(*(std::begin(m_slots) + m_free_head))};
...
constexpr void slot_map<T, Token, Container>::push_tail(size_type tail_index) {
  ...
  // This code sets the current tail to point to tail_index as the element after
  // it, then sets tail_index to point to itself signifying the last element in
  // the list.
...
constexpr bool slot_map<T, Token, Container>::key_valid(const key_type &key) {
  return this->key_generation(key)
         == this->key_generation(*(std::begin(m_slots) + key_index(key)));
}
...
constexpr void slot_map<T, Token, Container>::grow_slots() {
  ...
  this->key_index(m_slots.back())
    = static_cast<key_size_type>(m_slots.size() - 1);
...
template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::key_size_type &
slot_map<T, Token, Container>::key_index(key_type &key) {
  return std::get<0>(key);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr const typename slot_map<T, Token, Container>::key_size_type &
slot_map<T, Token, Container>::key_index(const key_type &key) const {
  return std::get<0>(key);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::key_generation_type &
slot_map<T, Token, Container>::key_generation(key_type &key) {
  return std::get<1>(key);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr const typename slot_map<T, Token, Container>::key_generation_type &
slot_map<T, Token, Container>::key_generation(const key_type &key) const {
  return std::get<1>(key);
}

template <typename T, typename Token, template <typename...> class Container>
constexpr typename slot_map<T, Token, Container>::key_size_type &
slot_map<T, Token, Container>::element_index(key_type &key) {
  return this->key_index(std::begin(m_slots) + this->key_index(key));
}

template <typename T, typename Token, template <typename...> class Container>
constexpr const typename slot_map<T, Token, Container>::key_size_type &
slot_map<T, Token, Container>::element_index(const key_type &key) const {
  return this->key_index(*(std::begin(m_slots) + this->key_index(key)));
}
...
Is leaving the overflow for generation counter up to the user provided type
fine? (default is do nothing) LEWG: Yes

Should clear reset generation counters (potential ABA break) or preserve them?
This would be semantically different from erase(begin(), end()). SG14: Yes



Discussion: accessing the underlying container. Desirable, but can break the
container if insert or erase happens

- address substitutability with map and unordered_map

- add small_vector and static_vector as desirable underlying container types,
thus adapter
