...
/************************************************************************************/
struct WeaponSystem {
  EntityRef FireWeapon( World &world, const Weapon &weapon, const Transform &tf, const ParentRef *parent = nullptr ) const {
...
struct ReloadSystem {
  void Process( Weapon &weapon, ReloadableClip &clip, const InputSource &input ) const {
...
struct ChargeWeaponSystem {
  void Process( Weapon &weapon, const InputSource &input, ChargeWeapon &cw, const Transform &tf ) const {
    weapon.TimeSinceFired = 0.f;
    if( !input.KeyDown( weapon.FireKey ) ) {
      this->TryFireWeapon( weapon, cw, cw.MinChargeTime );
      if( !cw.LoseChargeOverTime ) cw.CurrentCharge = 0.f;
      else cw.CurrentCharge -= std::min(Dt, cw.CurrentCharge); // Assuming Currentcharge can't go below 0
    }
    else {
      cw.CurrentCharge += Dt;
