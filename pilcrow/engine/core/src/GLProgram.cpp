...
std::string GLProgram::Directory( ) const {
  return ShaderPath( );
}
unsigned GLProgram::ID( ) const { 
  return m_ProgramID; 
}
...
bool GLProgram::SetUniform( const std::string &name, float x, float y, float z, float w ) {
...
const GLProgram::ShaderVariable * GLProgram::GetVariable( const std::string &name ) const {
...
bool GLProgram::LoadImpl( ) {
  ...
  if( success ) {
    ...
    success = Check( );
    this->ErrorCheck( );
    if( !success ) glDeleteProgram( m_ProgramID );
    else {
      this->GetAttributes( );
      this->ErrorCheck( );
      this->GetUniforms( );
...
bool GLProgram::Check( ) const {
  ...
  if( !success ) {
    ...
    std::cout << "An error has occurred on the program: "
      << Filename( )
      << ".\nInfo:\n"
      << info.data( )
      << std::endl;
...
void GLProgram::GetAttributes( ) {
  ...
  for( unsigned i{ 0 }; ( int )i < count; ++i ) {
    ...
    GLsizei namelength;
    sv.location = i;
    glGetActiveAttrib( this->ID( ), i, ( GLsizei )buffer.size( ), &namelength, &sv.size, &sv.type, buffer.data( ) );
    sv.name = buffer.data( );
...
void GLProgram::GetUniforms( ) {
  ...
  for( unsigned i{ 0 }; ( int )i < count; ++i ) {
    ...
    GLsizei namelength;
    sv.location = i;
    glGetActiveUniform( this->ID( ), i, ( GLsizei )buffer.size( ), &namelength, &sv.size, &sv.type, buffer.data( ) );
    sv.name = buffer.data( );
    m_variables[ sv.name ] = sv;
  }
}
